...
class TestDroneEnv(unittest.TestCase):
    """Test cases for DroneEnv version 1.0.0"""
    ...
    def assertEnvironmentVersion(self):
        """Verify environment version"""
        current_version = self.env.get_version()
        self.assertEqual(
            current_version,
            self.EXPECTED_VERSION,
            f"Environment version mismatch. Expected {self.EXPECTED_VERSION}, got {current_version}"
        )
    ...
    def test_reset_function(self):
        ...
        custom_timeout = 5.0
        observation, info = self.env.reset(options={'timeout': custom_timeout})
        print("✓ Custom timeout reset verified")
    ...
    def test_reward_function(self):
        ...
        print(f"Far from target reward: {far_reward}")
        ...
        print(f"High velocity reward: {velocity_reward}")
    ...
    def test_full_episode(self):
        ...
        while steps < max_steps:
            # Simple policy: try to reach target height
            ...
            print(f"Step {steps}: Height={height:.2f}m, Velocity={velocity:.2f}m/s, Reward={reward:.2f}")
